...
export default function DeviceFeedBackAvgRatingPieChart() {
  ...
  const allDevices = Array.from({ length: 5 }, (_, i) => i + 1).map(
    (deviceId) => {
      ...
      return {
        deviceId,
        averageRating: deviceEntry ? deviceEntry.averageRating.toFixed(2) : '0.00', // Set to 0 if not found
      };
    }
  );
  ...
  const downloadDataAsExcel = () => {
    const result = window.confirm("Do you want to proceed Download Average Ratings for device?");
    if(result){
